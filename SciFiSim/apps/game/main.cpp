...
int main() {
    ...
    sim::World world(20, 15, db.resourceCount(), popCfg, loaded.startStock);
    ...
    while (!WindowShouldClose()) {
        ...
        if (IsKeyPressed(KEY_S)) {
            if (sim::saveWorld(world, "output/save.bin"))
                reloadMsg = "saved";
            else
                reloadMsg = "save FAILED";
            reloadMsgTimer = 3.0f;
        }
        if (IsKeyPressed(KEY_L)) {
            if (sim::loadWorld(world, "output/save.bin"))
                reloadMsg = "loaded";
            else      
                reloadMsg = "load FAILED";
            reloadMsgTimer = 3.0f;
        }
