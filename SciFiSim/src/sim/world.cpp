...
World::World(int width, int height, int resourceCount, PopConfig popCfg,
             const std::vector<RecipeInput>& startStock)
    : width_(width), height_(height),
      tiles_(static_cast<size_t>(width) * height),
      stock_(static_cast<size_t>(resourceCount), 0),
      popCfg_(popCfg) {
    tiers_.resize(popCfg_.tiers.size());
    if (!tiers_.empty()) { tiers_[0].count = 10; tiers_[0].satisfaction = 100; }

    for (const RecipeInput& s : startStock) {
        if (s.res >= 0 && s.res < static_cast<int>(stock_.size()))
            stock_[static_cast<size_t>(s.res)] = s.amount;
    }
}
...
std::uint64_t World::hashState() const {
    ...
    for (const Tile& t : tiles_) {
        mix(static_cast<std::uint64_t>(t.building));
        mix(static_cast<std::uint64_t>(t.productionTick));
    }

    for (const TierState& ts : tiers_) {
        mix(static_cast<std::uint64_t>(ts.count));
        mix(static_cast<std::uint64_t>(ts.satisfaction));
        mix(static_cast<std::uint64_t>(ts.goodStreak));
        mix(static_cast<std::uint64_t>(ts.badStreak));
        mix(static_cast<std::uint64_t>(ts.upgradeStreak));
        mix(static_cast<std::uint64_t>(ts.downgradeStreak));
    }
