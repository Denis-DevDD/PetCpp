...
class World {
public:
    World(int width, int height, int resourceCount, PopConfig popCfg,
        const std::vector<RecipeInput>& startStock = {});

    std::uint64_t hashState() const;
    ...
    friend bool saveWorld(const World& w, const std::string& path);
    friend bool loadWorld(World& w, const std::string& path);
